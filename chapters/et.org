...
- /tmp/et.<basename>.<task-file>.<task> such as et.bm-core.chapters.dev.main.build.sh.
...
#+begin_src clojure :tangle ../src/jakub_stastny/et/runner.clj :mkdirp yes
  ...
  (def font-effects {:normal 0 :bold 1})
  ...
  (defn built-in-task--tasks
    ...
    (doseq [path args]
      ...
      (doseq [{:keys [name opts]} (org/load-tasks path)]
        (if (opts :doc)                     ; Ignore private tasks.
  ...
  (defn run-task [task config]
    ...
    (echo "Task definition " (pr-str task))
    (cond
      ;; Built-in task.
      (get-in task [:task :fn])
