        ...
        - name: Tangle Org files
          run: |
            for file in chapters/*.org; do
              emacs --batch --load="$file" --funcall org-babel-tangle
            done
...
* TODO Related project: tangler
- So I don't have to install and set up Emacs in order to get the files tangled on the CI.
