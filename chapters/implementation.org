...
#+begin_src clojure :tangle ../src/jakub_stastny/et/utils.clj :mkdirp yes
  ...
  (defn rethrow
    ...
    (if (instance? clojure.lang.ExceptionInfo error)
      (throw (ex-info
              (str "rethrow " (or (ex-message error) "unknown error"))
              (merge {:rethrown true} (ex-data error))
              error))
      (throw (ex-info (str error) {:rethrown true}))))
#+end_src
...
#+begin_src clojure :tangle ../src/jakub_stastny/et/config.clj :mkdirp yes
  ...
  (defn task?
    "..."
    [example]
    (or (= ((example :opts) :task) (symbol "yes"))
        ((example :opts) :doc)))
...
#+begin_src clojure :tangle ../src/jakub_stastny/et/org.clj :mkdirp yes
  ...
  (defn- no-such-task [{:keys [task-name]} message]
    (abort (format "No such task %s.\n\n%s" task-name message)))
  ...
  (defn run-task
    ...
    (let [examples (load-tasks (task-def :path))
          tasks (filter #((config :task?) %) examples)
          task (first (filter #(= (task-def :name) (% :name)) tasks))]
      (if task
        (exec-task (conj task-def task) config)
        (if (empty? tasks)
          (no-such-task task-def (format "No tasks available in %s." (:path task-def)))
          (let [available-tasks (str/join ", " (map #(% :name) tasks))]
            (no-such-task task-def (colour :green "Available tasks:") available-tasks "."))))))
#+end_src
