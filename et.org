...
#+begin_src clojure :tangle src/jakub_stastny/et/org.clj :mkdirp yes
  ...
  (defn get-ext
    "..."
    [lang] (or (get exts (keyword lang)) lang))

  (defn get-command
    "..."
    [lang] (or (get commands (keyword lang)) lang))

  (defn write-script
    "....."
    [{:keys [name lang lines opts]}]
    (let [shebang (if (opts :shebang)
                    (str/split (str/replace (opts :shebang) #"#!" "") #"\s+")
                    (str/split (get-command lang) #"\s+"))
          script-name (str "/tmp/" name "." (get-ext lang))]
  ...
  (defn exec-task
    ...
    (let [command (write-script task) res (apply sh command)]
      (println)
