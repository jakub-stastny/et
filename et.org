...
#+begin_src clojure :tangle src/jakub-stastny/et/runner.clj :mkdirp yes
      (ns jakub-stastny.et.runner
        "")

      ; BB has fs/exists? "str"
      ...
      (defn run [custom-config args]
        (let [config (conj default-config custom-config)]
          (prn config)
          (prn args)
          (let [task-defs ((config :args-to-tasks) args)]
               )
          ))
...
#+begin_src clojure :tangle src/jakub-stastny/et/parser.clj :mkdirp yes
