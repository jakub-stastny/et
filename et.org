...
#+begin_src clojure :tangle src/jakub_stastny/et/runner.clj :mkdirp yes
    ...
    (def built-in-tasks
      {:tasks #(println "Task: tasks")
       :help #(println "Task: help")})
  ...
  ;;   (println "Show all ..."))
    ...
    (def default-config
      ...
      ; :defs [{:task {:fn jakub-stastny.et.runner/built-in-tasks/tasks]}}]

      ; clj -M -m jakub-stastny.et.runner -T et-examples.org
      ; :defs ({:task {:name :tasks, :fn #object[jakub_stastny.et.runner$fn__149 0x3b0ca5e1 "jakub_stastny.et.runner$fn__149@3b0ca5e1"]}, :args ("et-examples.org")})
      {:args-to-tasks args-to-tasks})

    (defn run-task [task]
      (println (str "~ Running task " (prn-str task)))
      (cond
        ;; Built-in task.
        (get-in task [:task :fn])
        ((get-in task [:task :fn]))
        ...
        ;; Org-defined task.
        (and (task :path) (task :name))
        (et.org/run-task (task :path) (task :name))
