...
#+begin_src sh :task yes :doc "...."
  # clojure -M -m jakub-stastny.et.runner README.org build
  # java -jar target/et.jar README.org build
...
#+name: deploy
#+begin_src sh :task yes :doc "...."
  scp target/et-*-standalone.jar home:/bm/www/jar/et.jar
#+end_src
...
*** TODO Remove :task yes from the task map once filtered out
...
#+begin_src clojure :tangle src/jakub_stastny/et/org.clj :mkdirp yes
  ...
  (defn exec-task
    ...
    (let [exec-fn (config :exec-fn)
          command (write-script task config)
